  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($buttons as $button) {
      ...
      if($button->getKey() == 'finish') {
        // @todo: fix watchdog warnings inside IEF
        ElementSubmit::addCallback($form[$wrapper]['actions'][$button->getKey()], $form);
      }
    }
    ...
    if (empty($triggering_element['#skip_validation']) && $fields_validators = $this->step->getFieldsValidators()) {
      ...
      foreach ($fields_validators as $field => $validators) {
        ...
        foreach ($validators as $validator) {
          /** @var \Drupal\stacks\WidgetAdmin\Validator\ValidatorInterface $validator */
          // Do not validate if it's not set to reuse
          if ($field == 'widget_name' && $validator instanceof ValidatorRequired && !$reusable) {
            continue;
          }
