...
use Drupal\revslider\Model\Template;
...
//$all_slider = $tmp_slider->getArrSliders();
        ...
        <div id="template_area">
            <div id="template_header_part">
                <h2><span class="revlogo-mini"
                          style="margin-right:15px;"></span><?php echo t('Slider Template Library') ?></h2>
			...
			<span id="template_filter_buttons_wrapper" style="display:table-cell;vertical-align:top">	
                ...
                if (!empty($author_template_slider) && is_array($author_template_slider)) {
                    foreach ($author_template_slider as $name => $v) {
                        ?>
                        <span data-type="temp_<?php echo RevSliderFunctions::sanitize_title($name); ?>"
                              class="template_filter_button"><?php echo RevSliderFunctions::esc_attr($name); ?></span>
                        <?php
                    }
                }
                ?>
                <span style="display:none" id="selected_template_package_title">Light Content Block Page</span>
				<span style="display:none" id="leave_selected_template_package"><?php echo t('Back') ?></span>
				<span class="template_filter_button selected"
                      data-type="temp_all"><?php echo t('All Templates') ?></span>
				<span class="template_filter_button" data-type="template_free"><?php echo t('Free Templates') ?></span>
				<span class="template_filter_button"
                      data-type="template_premium"><?php echo t('Premium Templates') ?></span>
				<span class="template_filter_button"
                      data-type="template_package_parent"><?php echo t('Packages') ?></span>
				<span class="template_filter_button" data-type="temp_slider"><?php echo t('Slider') ?></span>
				<span class="template_filter_button" data-type="temp_carousel"><?php echo t('Carousel') ?></span>			
				<span class="template_filter_button" data-type="temp_hero"><?php echo t('Hero') ?></span>
				<span class="template_filter_button"
                      data-type="temp_notinstalled"><?php echo t('Not Installed') ?></span>
				<span class="template_filter_button" data-type="temp_socialmedia"><?php echo t('Social Media') ?></span>
				<span class="template_filter_button" data-type="temp_postbased"><?php echo t('Post-Based') ?></span>
				<span class="template_filter_button temp_new_udpated"
                      data-type="temp_newupdate"><?php echo t('New / Updated') ?></span>
			</span>
                    <span style="display:table-cell;vertical-align:top;text-align:right">
				<span class="rs-reload-shop"><i class="eg-icon-arrows-ccw"></i><?php echo t('Update Library') ?></span>
			</span>
                </div>
            </div>
        ...
        <script>
            ...
            function initTemplateSliders() {
                ...
                jQuery('body').on("click", '.revolution-templatebutton', function () {
                    var btn = jQuery(this);
                    jQuery('.revolution-template-groups').each(function () {
                        jQuery(this).hide();
                    });
            ...
            <?php
            if(!is_null(RevSliderFunctions::getRequestVariable('update_shop', null))){
            ?>
        ...
        <div id="dialog_import_template_slider" title="<?php echo t("Import Template Slider") ?>"
             class="dialog_import_template_slider" style="display:none">
            <form id="form-import-online-slider-local" action="<?php echo RevSliderFunctions::ajax_url(); ?>"
                ...
                <p><?php echo t('Please select the corresponding zip file from the download packages import folder called') ?>
                    :</p>
                ...
                ?>
                <p class="import-file-wrapper"><input type="file" size="60" name="import_file"
                                                      class="input_import_slider "></p>
                <span style="margin-top:45px;display:block"><input type="submit"
                                                                   class="rs-import-slider-button button-primary revblue tp-be-button"
                                                                   value="<?php echo t("Import Template Slider") ?>"></span>
                <span class="tp-clearfix"></span>
                <span style="font-weight: 700;"><?php echo t("Note: style templates will be updated if they exist!") ?></span>
        ...
        <div id="dialog_import_template_slider_from" title="<?php echo t("Import Template Slider") ?>"
             class="dialog_import_template_slider_from" style="display:none">
            <?php echo t('Import Slider from local or from ThemePunch server?') ?>
            <form action="<?php echo RevSliderFunctions::ajax_url(); ?>" enctype="multipart/form-data" method="post"
                  name="rs-import-template-from-server" id="rs-import-template-from-server">
                <input type="hidden" name="action" value="revslider_ajax_action">
        ...
        <div id="dialog_import_template_slider_info" title="<?php echo t('Importing Status') ?>"
             class="dialog_import_template_slider_info" style="display:none">
            <!-- ADD INFOS HERE ON DEMAND -->
            <div class="revslider_logo_rotating">
                <div class="revslidercycle"></div>
            </div>
            <div id="install-slider-counter-wrapper"><span id="install-slider-counter"></span></div>
            <div id="nowinstalling_label"><?php echo t('Now Installing') ?></div>
            <div id="import_dialog_box_action"></div>
            <div id="import_dialog_box"></div>

        </div>

        <div id="dialog_import_template_slider_page_template" title="<?php echo t("Create Blank Page") ?>"
             class="dialog_import_template_slider_page_template" style="display:none">
            <?php echo t('Create a Blank Demo Page with this Slider added to it?'); ?>
        </div>
        <?php
        return ob_get_clean();
    }
