...
class Output extends RevSliderElementsBase
{
...
//            $slver = apply_filters('revslider_remove_version', RevSliderGlobals::SLIDER_REVISION);
            ...
            $htmlBeforeSlider .= Operations::printCleanFontImport();
...
//                $htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
            ...
            $bannerWidth = $this->slider->getParam("width",null,Slider::VALIDATE_NUMERIC,"Slider Width");
            $bannerHeight = $this->slider->getParam("height",null,Slider::VALIDATE_NUMERIC,"Slider Height");
            ...
            $backgroundColor = RevSliderFunctions::esc_attr(trim($this->slider->getParam('background_color')));
            if(!empty($backgroundColor)) {
                $containerStyle .= 'background:'.TPColorpicker::get($backgroundColor).';';
            }

            //set padding
            $containerStyle .= 'padding:'.RevSliderFunctions::esc_attr($this->slider->getParam('padding','0')).'px;';

            //set margin:
            if($sliderType != 'fullscreen'){

                if($sliderPosition != 'center'){
                    $containerStyle .= 'margin-left:'.RevSliderFunctions::esc_attr($this->slider->getParam('margin_left', '0', Slider::FORCE_NUMERIC)).'px;';
                    $containerStyle .= 'margin-right:'.RevSliderFunctions::esc_attr($this->slider->getParam('margin_right', '0', Slider::FORCE_NUMERIC)).'px;';
                }

                $containerStyle .= 'margin-top:'.RevSliderFunctions::esc_attr($this->slider->getParam('margin_top', '0', Slider::FORCE_NUMERIC)).'px;';
                $containerStyle .= 'margin-bottom:'.RevSliderFunctions::esc_attr($this->slider->getParam('margin_bottom', '0', Slider::FORCE_NUMERIC)).'px;';
            }
            ...
            if($showBackgroundImage == 'true' || $showBackgroundImage == 'on'){
                $backgroundImage = RevSliderFunctions::esc_attr($this->slider->getParam('background_image'));
                $backgroundFit = RevSliderFunctions::esc_attr($this->slider->getParam('bg_fit', $this->slider->getParam('def-background_fit', 'cover')));
                $backgroundRepeat = RevSliderFunctions::esc_attr($this->slider->getParam('bg_repeat', $this->slider->getParam('def-bg_repeat', 'no-repeat')));
                $backgroundPosition = RevSliderFunctions::esc_attr($this->slider->getParam('bg_position', $this->slider->getParam('def-bg_position', 'center center')));
            ...
            $maxWidth = $this->slider->getParam('max_width', '0', Slider::FORCE_NUMERIC);
            ...
            $progress_style = ' style="height: '.RevSliderFunctions::esc_attr($progress_height).'px; background: '.TPColorpicker::convert($progressbar_color, $progress_opa).';"';
            ...
            $add_alias = '';
            if(RevSliderFunctions::is_super_admin() || RevSliderFunctions::is_admin_bar_showing()){
                if(RevSliderFunctions::current_user_can('edit_theme_options')){
                    $add_alias = ' data-alias="'.RevSliderFunctions::esc_attr($this->slider->getAlias()).'"';
                }
            }
    ...
    public function putErrorMessage($message){
        ?>
        <div style="width:800px;height:300px;margin:0 auto 10px;">
            <div style=" line-height: 40px; font-size: 14px; color: #FFF; padding: 15px; background: #E74C3C; margin: 20px 0 auto auto;">
                <?php echo t("Revolution Slider Error"); ?>: <?php echo $message; ?>
            </div>
        </div>
    ...
    public function get_responsive_size(){
        $operations = new Operations();
        $arrValues = $operations->getGeneralSettingsValues();
        ...
        if($adv_resp_sizes == true){
            $width = $this->slider->getParam("width", 1240, Slider::FORCE_NUMERIC);
            $def = $width;
            $width .= ',';
            if($enable_custom_size_notebook == 'on'){
                $width.= $this->slider->getParam("width_notebook", 1024, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_notebook", 1024, Slider::FORCE_NUMERIC);
            }else{
                $width.= $def;
            }
            $width.= ',';
            if($enable_custom_size_tablet == 'on'){
                $width.= $this->slider->getParam("width_tablet", 778, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_tablet", 778, Slider::FORCE_NUMERIC);
            }else{
                $width.= $def;
            }
            $width.= ',';
            if($enable_custom_size_iphone == 'on'){
                $width.= $this->slider->getParam("width_mobile", 480, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_mobile", 480, Slider::FORCE_NUMERIC);
            }else{
                $width.= $def;
            }

            $height = $this->slider->getParam("height", 1240, Slider::FORCE_NUMERIC);
            $def = $height;
            $height .= ',';
            if($enable_custom_size_notebook == 'on'){
                $height.= $this->slider->getParam("height_notebook", 1024, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_notebook", 1024, Slider::FORCE_NUMERIC);
            }else{
                $height.= $def;
            }
            $height.= ',';
            if($enable_custom_size_tablet == 'on'){
                $height.= $this->slider->getParam("height_tablet", 778, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_tablet", 778, Slider::FORCE_NUMERIC);
            }else{
                $height.= $def;
            }
            $height.= ',';
            if($enable_custom_size_iphone == 'on'){
                $height.= $this->slider->getParam("height_mobile", 480, Slider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_mobile", 480, Slider::FORCE_NUMERIC);
            }else{
                $height.= $def;
            }
            ...
            return array(
                ...
                'height' => $this->slider->getParam("height", "868", Slider::FORCE_NUMERIC),
                'width' => $this->slider->getParam("width", "1240", Slider::FORCE_NUMERIC)
            );
        }
    }
    ...
    private function filterOneSlide($slides){
        ...
        if(strpos($oneSlideID, 'static_') !== false){
            ...
            $tmp_slides = RevSliderDB::instance(array(
    ...
    private function putJS($markup_export = false){
        ...
        $hideSliderAtLimit = $this->slider->getParam("hide_slider_under","0",Slider::VALIDATE_NUMERIC);
        if(!empty($hideSliderAtLimit))
            $hideSliderAtLimit++;

        $hideCaptionAtLimit = $this->slider->getParam("hide_defined_layers_under","0",Slider::VALIDATE_NUMERIC);;
        if(!empty($hideCaptionAtLimit))
            $hideCaptionAtLimit++;

        $hideAllCaptionAtLimit = $this->slider->getParam("hide_all_layers_under","0",Slider::VALIDATE_NUMERIC);;
        ...
        $swipe_velocity = $this->slider->getParam("swipe_velocity","0.7",Slider::VALIDATE_NUMERIC);
        $swipe_min_touches = $this->slider->getParam("swipe_min_touches","1",Slider::VALIDATE_NUMERIC);
...
//        $do_delay = apply_filters('revslider_add_js_delay', $do_delay);
        ...
        $csizes = $this->get_responsive_size();
        ...
        ?>
        <script type="text/javascript">
            var revapi<?php echo $sliderID; ?>,tpj;
            ...
            setREVStartSize(<?php
                ...
                if($this->slider->getParam("slider_type") == "fullscreen"){
                    echo ",";
                    echo " fullScreenAutoWidth:'". RevSliderFunctions::esc_attr($this->slider->getParam("autowidth_force","off")) ."',";
                    echo " fullScreenAlignForce:'". RevSliderFunctions::esc_attr($this->slider->getParam("full_screen_align_force","off")) ."',";
                    echo " fullScreenOffsetContainer:'". RevSliderFunctions::esc_attr($this->slider->getParam("fullscreen_offset_container","")) ."',";
                    echo " fullScreenOffset:'". RevSliderFunctions::esc_attr($this->slider->getParam("fullscreen_offset_size","")) ."'";
                }
            ...
            if($do_delay > 0){
                echo '			startDelay: '. RevSliderFunctions::esc_attr($do_delay) .','."\n";
            }

            echo '			sliderType:"'. RevSliderFunctions::esc_attr($slider_type) .'",'."\n";

            echo '			jsFileLocation:"'. RevSliderFunctions::asset('public/js/',false) .'",'."\n";
            ...
            echo '			sliderLayout:"'.$sl_layout.'",'."\n";
            echo '			dottedOverlay:"'. RevSliderFunctions::esc_attr($this->slider->getParam("background_dotted_overlay","none")).'",'."\n";
            echo '			delay:'.RevSliderFunctions::esc_attr($this->slider->getParam("delay","9000",Slider::FORCE_NUMERIC)) .','."\n";
...
//            do_action('revslider_fe_javascript_option_output', $this->slider);
            ...
            if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')){
                echo '			navigation: {'."\n";
                echo '				keyboardNavigation:"'. RevSliderFunctions::esc_attr($keyboard_enabled) .'",'."\n";
                echo '				keyboard_direction: "'. RevSliderFunctions::esc_attr($keyboard_direction) .'",'."\n";
                echo '				mouseScrollNavigation:"'. RevSliderFunctions::esc_attr($mousescroll_enabled) .'",'."\n";
                echo ' 							mouseScrollReverse:"'. RevSliderFunctions::esc_attr($mousescroll_reverse) .'",'."\n";

                if($slider_type !== 'hero')
                    echo '				onHoverStop:"'. RevSliderFunctions::esc_attr($this->slider->getParam("stop_on_hover","on")).'",'."\n";

                $add_comma = false;
                if($touch_enabled == 'on'){
                    $add_comma = true;
                    echo '				touch:{'."\n";
                    echo '					touchenabled:"'. RevSliderFunctions::esc_attr($touch_enabled).'",'."\n";
                    echo '					touchOnDesktop:"'. RevSliderFunctions::esc_attr($touch_enabled_desktop).'",'."\n";
                    echo '					swipe_threshold: '. RevSliderFunctions::esc_attr($swipe_velocity) .','."\n";
                    echo '					swipe_min_touches: '. RevSliderFunctions::esc_attr($swipe_min_touches) .','."\n";
                    echo '					swipe_direction: "'. RevSliderFunctions::esc_attr($swipe_direction) .'",'."\n";
                ...
                if($enable_arrows == 'on'){
                    ...
                    $add_comma = true;
                    echo '				arrows: {'."\n";
                    echo '					style:"'. RevSliderFunctions::esc_attr($navigation_arrow_style).'",'."\n";
                    echo '					enable:';
                    ...
                    if($hide_arrows_on_mobile === 'true') {
                        echo '					hide_under:'. RevSliderFunctions::esc_attr($this->slider->getParam('arrows_under_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    if($hide_arrows_over === 'true') {
                        echo '					hide_over:'. RevSliderFunctions::esc_attr($this->slider->getParam('arrows_over_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					hide_onleave:'.$arrows_always_on.','."\n";
                    if($arrows_always_on === 'true'){
                        echo '					hide_delay:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_arrows','200',Slider::FORCE_NUMERIC)).','."\n";
                        echo '					hide_delay_mobile:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_arrows_mobile','1200',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					tmp:\'';
                    echo preg_replace( "/\r|\n/", "", $arr_tmp);
                    echo '\','."\n";
                    ...
                    echo '						h_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_align_hor','left')) .'",'."\n";
                    echo '						v_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_align_vert','center')) .'",'."\n";
                    echo '						h_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_offset_hor','20',Slider::FORCE_NUMERIC)) .','."\n";
                    echo '						v_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_offset_vert','0',Slider::FORCE_NUMERIC))."\n";
                    echo '					},'."\n";
                    echo '					right: {'."\n";
                    echo (in_array($this->slider->getParam('rightarrow_position', 'slider'),array('layergrid','grid'))) ? '									container:"layergrid",'."\n" : '';
                    echo '						h_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_align_hor','right')).'",'."\n";
                    echo '						v_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_align_vert','center')).'",'."\n";
                    echo '						h_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_offset_hor','20',Slider::FORCE_NUMERIC)).','."\n";
                    echo '						v_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_offset_vert','0',Slider::FORCE_NUMERIC))."\n";
                    echo '					}'."\n";
                    echo '				}'."\n";
                }
                ...
                if($enable_bullets == 'on'){
                    ...
                    if($hide_bullets_on_mobile === 'true'){
                        echo '					hide_under:'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_under_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    if($hide_bullets_over === 'true'){
                        echo '					hide_over:'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_over_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					style:"'. RevSliderFunctions::esc_attr($navigation_bullets_style).'",'."\n";
                    echo '					hide_onleave:'.$bullets_always_on.','."\n";
                    if($bullets_always_on === 'true'){
                        echo '					hide_delay:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_bullets','200',Slider::FORCE_NUMERIC)).','."\n";
                        echo '					hide_delay_mobile:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_bullets_mobile','1200',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					direction:"'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_direction','horizontal')).'",'."\n";
                    echo (in_array($this->slider->getParam('bullets_position', 'slider'),array('layergrid','grid'))) ? '									container:"layergrid",'."\n" : '';
                    echo '					h_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_align_hor','right')).'",'."\n";
                    echo '					v_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_align_vert','center')).'",'."\n";
                    echo '					h_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_offset_hor','20',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					v_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_offset_vert','0',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					space:'. RevSliderFunctions::esc_attr($this->slider->getParam('bullets_space','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					tmp:\'';
                    echo preg_replace( "/\r|\n/", "", $bul_tmp);
                    echo '\''."\n";
                    echo '				}'."\n";
                ...
                if($enable_thumbnails == 'on'){
                    ...
                    $add_comma = true;
                    echo '				thumbnails: {'."\n";
                    echo '					style:"'. RevSliderFunctions::esc_attr($thumbnails_style).'",'."\n";
                    echo '					enable:';
                    echo ($this->slider->getParam('enable_thumbnails','off') == 'on') ? 'true' : 'false';
                    echo ','."\n";
                    echo ($this->slider->getParam('rtl_thumbnails','off') == 'on') ? '								rtl:true,'."\n" : '';
                    echo '					width:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumb_width','100',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					height:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumb_height','50',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					min_width:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumb_width_min','100',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					wrapper_padding:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_padding','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					wrapper_color:"'.TPColorpicker::convert(RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_wrapper_color','transparent')),$this->slider->getParam('thumbnails_wrapper_opacity',false)).'",'."\n";
                    echo '					tmp:\'';
                    echo preg_replace( "/\r|\n/", "", $thumbs_tmp);
                    echo '\','."\n";
                    echo '					visibleAmount:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumb_amount','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					hide_onmobile:'.$hide_thumbs_on_mobile.','."\n";
                    if($hide_thumbs_on_mobile === 'true'){
                        echo '					hide_under:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbs_under_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    if($hide_thumbs_over === 'true'){
                        echo '					hide_over:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbs_over_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					hide_onleave:'.$thumbs_always_on.','."\n";
                    if($thumbs_always_on === 'true'){
                        echo '					hide_delay:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_thumbs','200',Slider::FORCE_NUMERIC)).','."\n";
                        echo '					hide_delay_mobile:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_thumbs_mobile','1200',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					direction:"'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnail_direction','horizontal')).'",'."\n";
                    echo '					span:';
                    echo ($this->slider->getParam('span_thumbnails_wrapper','off') == 'on') ? 'true' : 'false';
                    echo ','."\n";
                    echo '					position:"'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_inner_outer','inner')).'",'."\n";
                    ...
                    echo '					space:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_space','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					h_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_align_hor','left')).'",'."\n";
                    echo '					v_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_align_vert','center')).'",'."\n";
                    echo '					h_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_offset_hor','20',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					v_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_offset_vert','0',Slider::FORCE_NUMERIC))."\n";
                    echo '				}'."\n";
                }
                if($enable_tabs == 'on'){
                    ...
                    $add_comma = true;
                    echo '				tabs: {'."\n";
                    echo '					style:"'. RevSliderFunctions::esc_attr($tabs_style).'",'."\n";
                    echo '					enable:';
                    echo ($this->slider->getParam('enable_tabs','off') == 'on') ? 'true' : 'false';
                    echo ','."\n";
                    echo ($this->slider->getParam('rtl_tabs','off') == 'on') ? '								rtl:true,'."\n" : '';
                    echo '					width:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_width','100',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					height:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_height','50',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					min_width:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_width_min','100',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					wrapper_padding:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_padding','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					wrapper_color:"'.TPColorpicker::convert(RevSliderFunctions::esc_attr($this->slider->getParam('tabs_wrapper_color','transparent')),$this->slider->getParam('tabs_wrapper_opacity',false)).'",'."\n";
                    echo '					tmp:\'';
                    echo preg_replace( "/\r|\n/", "", $tabs_tmp);
                    echo '\','."\n";
                    echo '					visibleAmount: '. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_amount','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					hide_onmobile: '.$hide_tabs_on_mobile.','."\n";
                    if($hide_tabs_on_mobile === 'true'){
                        echo '					hide_under:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_under_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    if($hide_tabs_over === 'true'){
                        echo '					hide_over:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_over_hidden','0',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					hide_onleave:'.$tabs_always_on.','."\n";
                    if($tabs_always_on === 'true'){
                        echo '					hide_delay:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs','200',Slider::FORCE_NUMERIC)).','."\n";
                        echo '					hide_delay_mobile:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs_mobile','1200',Slider::FORCE_NUMERIC)).','."\n";
                    }
                    echo '					hide_delay:'. RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs','200',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					direction:"'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_direction','horizontal')).'",'."\n";
                    echo '					span:';
                    echo ($this->slider->getParam('span_tabs_wrapper','off') == 'on') ? 'true' : 'false';
                    echo ','."\n";
                    echo '					position:"'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_inner_outer','inner')).'",'."\n";
                    ...
                    echo '					space:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_space','5',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					h_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_align_hor','left')).'",'."\n";
                    echo '					v_align:"'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_align_vert','center')).'",'."\n";
                    echo '					h_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_offset_hor','20',Slider::FORCE_NUMERIC)).','."\n";
                    echo '					v_offset:'. RevSliderFunctions::esc_attr($this->slider->getParam('tabs_offset_vert','0',Slider::FORCE_NUMERIC))."\n";
                    echo '				}'."\n";
                }
                echo '			},'."\n";
            ...
            if($slider_type == 'carousel'){
                ...
                $car_space = $this->slider->getParam('carousel_space',0,Slider::FORCE_NUMERIC);
                $car_stretch = $this->slider->getParam('carousel_stretch','off');
                $car_all_layers = $this->slider->getParam('showalllayers_carousel','off');
                $car_maxitems = $this->slider->getParam('carousel_maxitems',5,Slider::FORCE_NUMERIC);
                ...
                $car_speed = $this->slider->getParam('carousel_speed','800',Slider::FORCE_NUMERIC);
                ...
                $car_maxrotation = $this->slider->getParam('carousel_maxrotation',90,Slider::FORCE_NUMERIC);
                ...
                $car_scaledown = $this->slider->getParam('carousel_scaledown',50,Slider::FORCE_NUMERIC);
                if($car_scaledown > 100) $car_scaledown = 100;

                $car_borderr = $this->slider->getParam('carousel_borderr',0,Slider::FORCE_NUMERIC);
                $car_borderr_unit = $this->slider->getParam('carousel_borderr_unit','px');

                $car_padding_top = $this->slider->getParam('carousel_padding_top',0,Slider::FORCE_NUMERIC);
                $car_padding_bottom = $this->slider->getParam('carousel_padding_bottom',0,Slider::FORCE_NUMERIC);

                echo '			carousel: {'."\n";
                if($car_borderr > 0){
                    echo '				border_radius: "'. RevSliderFunctions::esc_attr($car_borderr.$car_borderr_unit) .'",'."\n";
                }
                if($car_padding_top > 0){
                    echo '				padding_top: "'. RevSliderFunctions::esc_attr($car_padding_top) .'",'."\n";
                }
                if($car_padding_bottom > 0){
                    echo '				padding_bottom: "'. RevSliderFunctions::esc_attr($car_padding_bottom) .'",'."\n";
                }
                if($carousel_rotation == 'on'){
                    echo '				maxRotation: '. RevSliderFunctions::esc_attr($car_maxrotation).','."\n";
                    echo '				vary_rotation: "'. RevSliderFunctions::esc_attr($car_varyrotate).'",'."\n";
                }
                if($carousel_scale == 'on'){
                    echo '				minScale: '. RevSliderFunctions::esc_attr($car_scaledown).','."\n";
                    echo '				vary_scale: "'. RevSliderFunctions::esc_attr($car_varyscale).'",'."\n";
                }
                echo '				horizontal_align: "'. RevSliderFunctions::esc_attr($car_hpos).'",'."\n";
                echo '				vertical_align: "'. RevSliderFunctions::esc_attr($car_vpos).'",'."\n";
                echo '				fadeout: "'. RevSliderFunctions::esc_attr($car_fadeout).'",'."\n";
                if($car_fadeout == 'on'){
                    echo '				vary_fade: "'. RevSliderFunctions::esc_attr($car_varyfade).'",'."\n";
                }
                echo '				maxVisibleItems: '. RevSliderFunctions::esc_attr($car_maxitems).','."\n";
                echo '				infinity: "'. RevSliderFunctions::esc_attr($car_inf).'",'."\n";
                echo '				space: '. RevSliderFunctions::esc_attr($car_space).','."\n";
                echo '				stretch: "'. RevSliderFunctions::esc_attr($car_stretch).'",'."\n";
                echo ' 							showLayersAllTime: "'.RevSliderFunctions::esc_attr($car_all_layers).'",'."\n";
                echo ' 							easing: "'.RevSliderFunctions::esc_attr($car_easing).'",'."\n";
                echo ' 							speed: "'.RevSliderFunctions::esc_attr($car_speed).'"'."\n";
            ...
            $viewport_area = $this->slider->getParam('viewport_area', 80, Slider::FORCE_NUMERIC);
            if($label_viewport === 'on'){
                echo '			viewPort: {'."\n";
                echo '				enable:true,'."\n";
                echo '				outof:"'.RevSliderFunctions::esc_attr($viewport_start).'",'."\n";
                echo '				visible_area:"'.RevSliderFunctions::esc_attr($viewport_area).'%",'."\n";
                echo '				presize:';
            ...
            echo '			lazyType:"'. RevSliderFunctions::esc_attr($lazyLoad).'",'."\n";
            ...
            if($use_scrolleffect =='on') {
                ...
                if($this->slider->getParam('fade_scrolleffect','off') !== 'off')
                    echo '				fade:"'. RevSliderFunctions::esc_attr($this->slider->getParam('fade_scrolleffect','off')) .'",'."\n";

                if($this->slider->getParam('blur_scrolleffect','off') !== 'off')
                    echo '				blur:"'. RevSliderFunctions::esc_attr($this->slider->getParam('blur_scrolleffect','off')) .'",'."\n";

                //if($this->slider->getParam('scale_scrolleffect','off') !== 'off')
                //echo '				scale:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scale_scrolleffect','off')) .'",'."\n";

                if($this->slider->getParam('grayscale_scrolleffect','off') !== 'off')
                    echo '				grayscale:"'. RevSliderFunctions::esc_attr($this->slider->getParam('grayscale_scrolleffect','off')) .'",'."\n";


                if($this->slider->getParam('scrolleffect_maxblur','10') !== '10')
                    echo '				maxblur:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_maxblur','10')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_bg','off') !== 'off')
                    echo '				on_slidebg:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_bg','off')) .'",'."\n";


                if($this->slider->getParam('scrolleffect_layers','off') !== 'off')
                    echo '				on_layers:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_layers','off')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_parallax_layers','off') !== 'off')
                    echo '				on_parallax_layers:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_parallax_layers','off')) .'",'."\n";


                if($this->slider->getParam('scrolleffect_static_layers','off') !== 'off')
                    echo '				on_static_layers:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_static_layers','off')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_static_parallax_layers','off') !== 'off')
                    echo '				on_parallax_static_layers:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_static_parallax_layers','off')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_direction','both') !== 'both')
                    echo '				direction:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_direction','both')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_multiplicator','1.3') !== '1.3')
                    echo '				multiplicator:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_multiplicator','1.3')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_multiplicator_layers','1.3') !== '1.3')
                    echo '				multiplicator_layers:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_multiplicator_layers','1.3')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_tilt','30') !== '30')
                    echo '				tilt:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_tilt','30')) .'",'."\n";

                if($this->slider->getParam('scrolleffect_off_mobile','on') !== 'on')
                    echo '				disable_on_mobile:"'. RevSliderFunctions::esc_attr($this->slider->getParam('scrolleffect_off_mobile','on')) .'",'."\n";
                echo '			},'."\n";
            }

            if($use_parallax == 'on'){
                echo '			parallax: {'."\n";
                echo '				type:"'. RevSliderFunctions::esc_attr($parallax_type) .'",'."\n";
                echo '				origo:"'. RevSliderFunctions::esc_attr($parallax_origo) .'",'."\n";
                echo '				speed:'. RevSliderFunctions::esc_attr($parallax_speed) .','."\n";
                echo '				speedbg:'. RevSliderFunctions::esc_attr($parallax_bg_speed) .','."\n";
                echo '				speedls:'. RevSliderFunctions::esc_attr($parallax_ls_speed) .','."\n";
                echo '				levels:['. RevSliderFunctions::esc_attr($parallax_level) .'],'."\n";

                if ($parallax_type == '3D') {
                    echo '				ddd_shadow:"'. RevSliderFunctions::esc_attr($parallax_ddd_shadow) .'",'."\n";
                    echo '				ddd_bgfreeze:"'. RevSliderFunctions::esc_attr($parallax_ddd_bgfreeze) .'",'."\n";
                    echo '				ddd_overflow:"'. RevSliderFunctions::esc_attr($parallax_ddd_overflow) .'",'."\n";
                    echo '				ddd_layer_overflow:"'. RevSliderFunctions::esc_attr($parallax_ddd_layer_overflow) .'",'."\n";
                    echo '				ddd_z_correction:'. RevSliderFunctions::esc_attr($parallax_ddd_zcorrection) .','."\n";
                    //echo '				ddd_path:"'. RevSliderFunctions::esc_attr($parallax_ddd_path) .'",'."\n";
                }
            ...
            if ($use_parallax != 'on' || ($use_parallax == 'on' && $parallax_type !='3D'))
                echo '			shadow:'. RevSliderFunctions::esc_attr($this->slider->getParam("shadow_type","2")) .','."\n";

            if($use_spinner == '-1'){
                echo '			spinner:"off",'."\n";
            }else{
                echo '			spinner:"spinner'. RevSliderFunctions::esc_attr($use_spinner).'",'."\n";
            }

            if($slider_type !== 'hero'){
                echo '			stopLoop:"'. RevSliderFunctions::esc_attr($stopSlider) .'",'."\n";
                echo '			stopAfterLoops:'. RevSliderFunctions::esc_attr($stopAfterLoops) .','."\n";
                echo '			stopAtSlide:'. RevSliderFunctions::esc_attr($stopAtSlide) .','."\n";
                echo '			shuffle:"'. RevSliderFunctions::esc_attr($this->slider->getParam("shuffle","off")) .'",'."\n";
            }

            echo '			autoHeight:"'. RevSliderFunctions::esc_attr($this->slider->getParam("auto_height", 'off')). '",'."\n";
            ...
            echo '			hideThumbsOnMobile:"'. RevSliderFunctions::esc_attr($hideThumbsOnMobile) .'",'."\n";
            echo '			hideSliderAtLimit:'. RevSliderFunctions::esc_attr($hideSliderAtLimit) .','."\n";
            echo '			hideCaptionAtLimit:'. RevSliderFunctions::esc_attr($hideCaptionAtLimit) .','."\n";
            echo '			hideAllCaptionAtLilmit:'. RevSliderFunctions::esc_attr($hideAllCaptionAtLimit) .','."\n";

            if($slider_type !== 'hero'){
                $start_with_slide_enable = $this->slider->getParam('start_with_slide_enable', 'off');
                if($start_with_slide_enable == 'on'){
                    echo '			startWithSlide:'. RevSliderFunctions::esc_attr($startWithSlide).','."\n";
                }
            }
            ...
            if($this->slider->getParam('ignore_height_changes', 'off') !== 'off'){
                echo '				ignoreHeightChanges:"'. RevSliderFunctions::esc_attr($this->slider->getParam('ignore_height_changes', 'off')).'",'."\n";
                echo '				ignoreHeightChangesSize:'. intval(RevSliderFunctions::esc_attr($this->slider->getParam('ignore_height_changes_px', '0'))).','."\n";
            }

            echo '				simplifyAll:"'. RevSliderFunctions::esc_attr($this->slider->getParam('simplify_ie8_ios4', 'off')).'",'."\n";

            if($slider_type !== 'hero')
                echo '				nextSlideOnWindowFocus:"'. RevSliderFunctions::esc_attr($this->slider->getParam('next_slide_on_window_focus', 'off')).'",'."\n";
...
//            add_action('wp_print_footer_scripts', array($this, 'add_inline_js'), 100);
        ...
        switch($use_spinner){
            case '1':
            case '2':
                if(!RevSliderFunctions::isAdminUser()){?><script>
        ...
        if($this->slider->getParam("custom_css", '') !== ''){
            if(!RevSliderFunctions::isAdminUser()){
                ?><script>
                    var htmlDivCss = decodeURI("<?php echo RevSliderCssParser::compress_css(rawurlencode(stripslashes($this->slider->getParam('custom_css', ''))));?>");
                    var htmlDiv = document.getElementById('rs-plugin-settings-inline-css');
                    if(htmlDiv) {
                        htmlDiv.innerHTML = htmlDiv.innerHTML + htmlDivCss;
                    }
...
//            }
    ...
    private function putSlides($gal_ids = array(), $order = array()){
        ...
        $mobile = (strstr($usragent,'Android') || strstr($usragent,'webOS')
            || strstr($usragent,'iPhone') ||strstr($usragent,'iPod')
            || strstr($usragent,'iPad') || strstr($usragent,'Windows Phone')
            || RevSliderFunctions::is_mobile()) ? true : false;
        ...
        if(empty($slides)){
            ...
            <div class="no-slides-text">
                ...
                if($this->slider->isSlidesFromPosts()){
                    echo t('No slides found, please add at least one Slide Template to the choosen language.');
                }else{
                    echo t('No slides found, please add some slides');
                }
                ?>
            </div>
            <?php
...
//        $do_static = apply_filters('revslider_enable_static_layers', true);
        ...
        foreach($slides as $slide){
            ...
            if($this->previewMode === false){ // do only if we are not in preview mode
                $ts = RevSliderFunctions::current_time('timestamp');
...
//                        $alt = get_post_meta( $img_id, '_wp_attachment_image_alt', true );
                        $alt = RevSliderFunctions::get_image_url_by_id($img_id);
                        break;
                    case 'file_name':
...
//                        $info = pathinfo($imageFilename);
                        ...
                        break;
                    case 'custom':
                        $alt = RevSliderFunctions::esc_attr($slide->getParam('alt_attr', ''));
                        break;
                }

                switch($title_type){
                    case 'media_library':
                        $url_image = RevSliderFunctions::get_image_url_by_id($img_id);
                        $img_title = basename($url_image);
                        break;
                    case 'file_name':
                        $imageFilename = $slide->getImageFilename();
                        $info = pathinfo($imageFilename);
                        ...
                        break;
                    case 'custom':
                        $img_title = RevSliderFunctions::esc_attr($slide->getParam('title_attr', ''));
                        break;
                }
                ...
                $urlSlideImage = RevSliderFunctions::esc_url($slide->getParam('slide_bg_external', ''));
                $alt = RevSliderFunctions::esc_attr($slide->getParam('alt_attr', ''));
                $img_title = RevSliderFunctions::esc_attr($slide->getParam('title_attr', ''));
                ...
                if($thumb_do == 'slider'){ //use the slider settings for width / height
                    ...
                    if($source_type == 'youtube' || $source_type == 'vimeo' ||
                        ...
                        if(empty($urlThumb)){	//try to get resized thumb
                            ...
                            $urlThumb =RevSliderFunctions::rev_aq_resize($url_img_link, $th_width, $th_height, true, true, true);
                        }else{
                            $urlThumb = RevSliderFunctions::rev_aq_resize($urlThumb, $th_width, $th_height, true, true, true);
                            if(empty($urlThumb)){
...
//                        $link = do_shortcode($slide->getParam('link', ''));
            ...
            $styleImage = '';
            $urlImageTransparent = RevSliderFunctions::asset('admin/images/transparent.png');
            ...
            if($lazyLoad != 'none'){
                ...
                $urlSlideImage = RevSliderFunctions::asset('admin/images/dummy.png');
            }
...
//                $slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), esc_attr($the_excerpt)).'"';
                ...
                $slide_title = ' data-title="'.stripslashes(RevSliderFunctions::esc_attr($slide->getParam("title","Slide"))).'"';
                $slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), RevSliderFunctions::esc_attr($slide->getParam('slide_description', ''))).'"';
            }
            $slide_id = $slide->getID();
            if($slide->getParam('slide_id','') !== '') $slide_id = RevSliderFunctions::esc_attr($slide_id);
            ...
            for($mi=1;$mi<=10;$mi++){
                ...
                if($pa !== ''){
                    $pa_limit = $slide->getParam('params_'.$mi.'_chars',10);
                    $pa = strip_tags($pa);
                    $pa = mb_substr($pa, 0, $pa_limit, 'utf-8');
                }
                $add_params .= ' data-param'.$mi.'="'.stripslashes(RevSliderFunctions::esc_attr($pa)).'"';
            }
...
//            do_action('revslider_add_li_data', $this->slider, $slide);
            ...
            $this->icon_sets = RevSliderFunctions::set_icon_sets(array());
            $this->customAnimations = Operations::getCustomAnimations('customin'); //get all custom animations
            $this->customEndAnimations = Operations::getCustomAnimations('customout'); //get all custom animations
            $this->startAnimations = Operations::getArrAnimations(false); //only get the standard animations
            $this->endAnimations = Operations::getArrEndAnimations(false); //only get the standard animations
...
//                do_action('revslider_add_static_layer_html', $this->slider);
    ...
    private function getSlideFullWidthVideoData(Slide $slide){
    ...
    public function checkIfStreamVideoExists(Slide $slide){
    ...
    public function putBackgroundVideo(Slide $slide){
        ...
        $setBase = RevSliderFunctions::get_url_base();
        ...
        switch($videoType){
            ...
            case 'twitter':
                ...
                if($youtube_id !== ''){
                    ...
                    $arguments.=';origin='.$setBase. RevSliderFunctions::getServerName().';';
...
//        $layer = apply_filters('revslider_putLayer_pre', $layer, $this, $row_group_uid, $static_slide, $special_type);
        ...
        if(trim($class) !== ''){
            ...
            if(!isset($this->in_class_usage[trim($class)])) $this->in_class_usage[trim($class)] = Operations::getCaptionsContentArray(trim($class));
        }
         ...
         **/
        $do_remove_inline = true;//apply_filters('revslider_remove_inline', true);
        ...
        $tcin = Operations::parseCustomAnimationByArray($layer, 'start', $frame_start);
        ...
        if($do_mask_in){
            $tmask = Operations::parseCustomMaskByArray($layer, 'start');
            if($tmask !== ''){
...
//                $html_toggle = do_shortcode(stripslashes($html_toggle));
            ...
            case 'image':
                ...
                if($img_size !== 'full'){
                    if($cur_img_id !== false && !empty($cur_img_id)){
                        $urlImage = RevSliderFunctions::wp_get_attachment_image_src($cur_img_id, $img_size);
                        $urlImage = $urlImage[0];
                    }
                }
                ...
                $img_h = '';
                if($cur_img_id !== false && !empty($cur_img_id)){
                    $img_data = RevSliderFunctions::wp_get_attachment_metadata( $cur_img_id );
                    if($img_data !== false && !empty($img_data)){
                        if($img_size !== 'full'){
                            if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
                ...
                switch($alt_option){
                    case 'media_library':
                        if($cur_img_id !== false){
                            $alt = RevSliderFunctions::get_image_url_by_id($cur_img_id);
                        }
                        break;
                    case 'file_name':
                ...
                if($lazyLoad != 'none' || $do_ll == 'force' && $do_ll !== 'ignore'){
                    ...
                    if($seo_opti === 'false' || $seo_opti === false){
                        ...
                        $urlImage = RevSliderFunctions::asset("admin/images/dummy.png");
                    }else{
            ...
            case 'audio':
                ...
                $urlAudio = RevSliderFunctions::esc_attr(RevSliderFunctions::getVal($videoData, 'urlAudio'));
            ...
            case 'video':
                ...
                switch($videoType){
                    ...
                    case 'youtube':
                        ...
                        $videoArgs.=';origin='.$setBase.RevSliderFunctions::getServerName().';';
            ...
            case 'column':
                ...
                $add_data .= ' data-columnwidth="'.RevSliderFunctions::esc_attr($size).'%"'."\n";
        ...
        if($has_trigger){
            ...
            $last_trigger_state = '			data-lasttriggerstate="'.RevSliderFunctions::esc_attr($trigger_memory).'"';
        ...
        if($endAnimation !== 'auto'){
            $tcout = Operations::parseCustomAnimationByArray($layer, 'end', $frame_end);
        ...
        if($do_mask_out){
            $tmask = Operations::parseCustomMaskByArray($layer, 'end');
            if($tmask !== ''){
...
//                $html_simple_link = apply_filters('rs_action_output_layer_simple_link', $html_simple_link, $action, $all_actions, $num, $this->slide, $this->slider, $a_events);
        ...
        echo '			data-type="'.RevSliderFunctions::esc_attr($type).'"'." \n";
...
//        global $post;
        $text = str_replace(array('%current_page_link%', '{{current_page_link}}'), RevSliderFunctions::current_link(), $text);
        $text = str_replace(array('%home_url%', '{{home_url}}'),RevSliderFunctions::home_url(), $text);
